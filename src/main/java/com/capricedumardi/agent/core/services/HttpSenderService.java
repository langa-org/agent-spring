...
public class HttpSenderService implements SenderService {
        ...
        while (attempt < maxRetryAttempts) {
            try {
                ...
                if (attempt < maxRetryAttempts) {
                    ...
                    // Mirror delay components in logs to quickly diagnose backoff/jitter behavior in production.
                    int maxDelay = Math.max(0, agentConfig.getHttpMaxRetryDelayMillis());
                    long baseDelay = Math.max(0L, agentConfig.getHttpBaseRetryDelayMillis());
                    long exponentialDelay = attempt >= 62 ? Long.MAX_VALUE : (baseDelay << Math.max(0, attempt));
                    int boundedBackoff = (int) Math.min((long) maxDelay, exponentialDelay);
                    int appliedJitter = Math.max(0, delay - boundedBackoff);

                    LangaPrinter.printError("HttpSenderService: Send failed (attempt " + attempt +
                        "/" + maxRetryAttempts + ") - delay[backoff=" + boundedBackoff +
                        "ms, jitter=" + appliedJitter + "ms, effective=" + delay + "ms]: " +
                        e.getMessage());
    ...
    private int calculateRetryDelay(int attempt) {
        ...
        // Saturating exponential backoff: avoid overflow for large attempts.
        long exponentialDelay;
        if (attempt >= 62) {
            exponentialDelay = Long.MAX_VALUE;
        } else {
            exponentialDelay = baseDelay << Math.max(0, attempt);
        }

        long boundedDelay = Math.min(exponentialDelay, (long) maxDelay);

        // Add bounded positive jitter [0, boundedDelay/2] to spread retries.
        long jitterRange = boundedDelay / 2;
        long jitter = jitterRange > 0
                ? ThreadLocalRandom.current().nextLong(jitterRange + 1)
                : 0L;

        long finalDelay = Math.min((long) maxDelay, boundedDelay + jitter);
        return (int) Math.max(0L, finalDelay);
    }
