# Langa Spring Agent - Complete Configuration Guide

## Overview

The Langa Spring Agent uses **flexible, multi-source configuration** to give you complete control over observability collection. Configuration can be provided via environment variables, configuration files, or system properties with clear priority rules.
...
|----------|-------------|----------------|
| `LOGGING_FRAMEWORK` | Which logging framework to use (use `logback` for Spring Boot apps) | `logback` (recommended), `log4j2`, `none` |
...
# Monitoring
export LANGA_MONITORING_ENABLED=true
...
Controls method-level monitoring auto-configuration for diagnostics.

Set `langa.monitoring.enabled` in the hosted application's `application.properties` or `application.yml` so the host service controls whether monitoring is enabled.

| Property | Env Variable | Default | Description |
|----------|-------------|---------|-------------|
| `langa.monitoring.enabled` | `LANGA_MONITORING_ENABLED` | `true` | Enable/disable Langa monitoring auto-configuration |
...
```
Loading Langa Spring Agent configuration...
...
========================================
  Langa Spring Agent Initialization Complete
========================================
```
...
**Recommendation for Spring Boot**: Use `LOGGING_FRAMEWORK=logback` as Spring Boot includes Logback by default.

| LOGGING_FRAMEWORK | Logback on Classpath | Log4j2 on Classpath | Result |
|-------------------|---------------------|---------------------|--------|
| `logback` ⭐ | ✅ Yes | ❌ No | ✅ Binds Logback appender (Spring Boot default) |
...
2. ✅ **Set LOGGING_FRAMEWORK=logback** for Spring Boot apps (Spring Boot uses Logback by default)
